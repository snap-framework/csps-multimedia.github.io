define(['jquery', 'settings-core', 'json!settings-general'], function($, CoreSettings, GeneralSettings) {
   'use strict';
   ...
   if (isMobile) {
      ...
      if (GeneralSettings.enableMicroLearning && window.innerWidth <= 1280) {
         isMobile = true;
   ...
   return {
      ...
      getArrayFromString: function(idString) {
         //CSPS-TD : problem stems from locking-system@159 (var subToLock = ObjSubUtils.findSub(Utils.getArrayFromString(target));)
         //Deal with potential error for now (this has to be fixed somewhere else eventually!)
         if(typeof idString !== "string"){
            console.error("expected String, got "+typeof idString);
            return false;
         }
