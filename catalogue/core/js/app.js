define([
   'underscore',
...
], function(_, $, labels, CoreSettings, GeneralSettings, Utils, Logger, History, Router, Legacy, MicroLearning, ScormController, DataController, Regular, LockingSystem, ObjNiveau, ObjSub, ObjSubUtils, PrintController, MobileInteractions, AdminMode, Navigation, Timeline, HelpController, SitemapController, Toolbox, Glossary, Resources, FavoriteController, ProgressController, googleAnalytics) {
	'use strict';
	...
	function navStructure() {
		this.version = "1.6.2"; //*DO NOT MODIFY. Auto-generated by build*//
		this.WETversion = "4.0.23"; //*DO NOT MODIFY. Auto-generated by build*//
		...
		*/
		...
		this.initModules = function() {		
			if (GeneralSettings.enableMicroLearning) {
			...
			if (GeneralSettings.trackAllPages) {
		...
		*/
		...
		this.initPostWetFixes = function() {
			...
			//add custom animation transition for popups
			//http://codepen.io/dimsemenov/pen/GAIkt
			$('.wb-lbx').on("mfpBeforeOpen", function(e, mfp) {
				mfp.st.removalDelay = 500; //delay removal by X to allow out-animation
				//check if we have recieved a $ element or a string
				var $html = mfp.items[0].src instanceof $
								? mfp.items[0].src
								: $(mfp.items[0].src);

				$html.addClass('mfp-with-anim');
				
				var effect = mfp.st.el.attr('data-effect') || "mfp-zoom-in";
				$html.attr('data-effect', effect);
				mfp.items[0].src = $html[0].outerHTML;
		      
		      mfp.st.mainClass = effect;
			});
		};
		...
		this.initMenuBar = function() {
			...
			//load sitemap
			if (GeneralSettings.showSitemap) {
		...
		this.initToolbox = function() {
			if ($(".toolbox").length > 0) {
				...
				//if (GeneralSettings.showResources) {
					this.resources = new Resources();
				//}
				//load glossary
				//if (GeneralSettings.showGlossary) {
					this.glossary = new Glossary();
				//}
				//favorites
				if (GeneralSettings.showFavorites) {
		...
		this.runAutomations = function() {
			...
			if ($(CoreSettings.contentContainer).find(".qs-elearning-activity").length > 0) {
				$.getScript(require.s.contexts._.config.baseUrl + "plugins/qsbeta.js");
			}
			...
			//fixes tabbing in jquery.magnificPopup details elements
			$(".wb-lbx").on("mfpAjaxContentAdded", function(e) {
				$(".mfp-content").find("details summary").attr("tabindex", 0);
				that.initWbs();
			});
		...
		this.loadSucessful = function() {
			...
			if (GeneralSettings.enableMicroLearning && !this.isFirstPage()) {
		...
		this.isLoaded = function(xhr) {
			...
			if (this.firstPageLoad) {
				...
				//google analytics check for setting
				if (typeof GeneralSettings.googleAnalyticsID !== 'undefined' && GeneralSettings.googleAnalyticsID.length>2) {
		...
		this.disableBacknext = function() {
			...
			if (this.isLastPage() && CoreSettings.loopLevel >= this.maxDepth) {
				$(".next").addClass("disabled").attr("tabindex", "-1").append("<span class='wb-inv'>("+labels.nav.disabled+")<span/>");
			}
		...
		--------------------------------------------------------------------------
								...
								modifyHeader
									xxxxxxxxxxxxxxx
		--------------------------------------------------------------------------*/
		this.fixMobileLightbox = function() {
		...
		this.modifyHeader = function() {
			var that = this;
			if (CoreSettings.navigationMode == 2 || GeneralSettings.enableMicroLearning) {
				...
				var titleHTML = (lang == "en") ? GeneralSettings.courseTitle_en : GeneralSettings.courseTitle_fr;
			...
			var seriesTitleHTML = (lang == "en") ? GeneralSettings.seriesTitle_en : GeneralSettings.seriesTitle_fr;
	...
	function initWbAdd(obj) {
		if ($(CoreSettings.contentContainer).find(obj).length > 0) {
			wb.add(obj);
		}
	}
