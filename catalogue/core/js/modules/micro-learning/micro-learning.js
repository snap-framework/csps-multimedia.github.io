define([
   ...
   'fullpage',
   'scrollto'
], function(_, $, BaseModule, Logger, Utils, labels, CoreSettings, GeneralSettings, TutorialController) {
   'use strict';
   ...
   return BaseModule.extend({
      ...
      onFirstModuleLoad: function() {
         ...
         if (!this.isMobile() && GeneralSettings.enableTutorial && !masterStructure.isFirstTimeLoaded) {
            this.launchTutorial();
         }
      },
      ...
      buildNav: function() {
         ...
         $('.micro-menu').remove();
         $('header[role=banner]').before("<section class='micro-menu col-md-3 col-sm-3 col-xs-3'><ul class='toc'></ul></section>");  
